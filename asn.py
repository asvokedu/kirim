...
class SignalDetector:
    ...
    def _fetch_trend_data(self, symbol: str) -> Optional[Dict]:
        """Mengambil data trend dari database menggunakan stored procedure"""
        with self.db_semaphore:
            conn = self._get_db_connection()
            if not conn:
                return None
            ...
            try:
                cursor = conn.cursor()
                # Eksekusi stored procedure dengan parameter output
                cursor.execute("""
                    DECLARE @signal NVARCHAR(10);
                    DECLARE @trend NVARCHAR(10);
                    DECLARE @change FLOAT;

                    EXEC dbo.sp_scalping_signal_15m
                        @symbol = ?,
                        @signal_out = @signal OUTPUT,
                        @trend_out = @trend OUTPUT,
                        @percent_change_out = @change OUTPUT;
                ...
                if row:
                    return {
                        ...
                        'trend': row.Trend,
                        'change': float(row.PercentChange) if row.PercentChange is not None else 0.0
                    }
                
                logger.warning(f"No trend data returned for {symbol}")
                return None
            except Exception as e:
                logger.error(f"Gagal mengambil trend data untuk {symbol}: {e}")
                return None
            finally:
                try:
                    conn.close()
                    ...
                    try:
                        ...
                        if trend_data:
                            with self.trend_lock:
                                # Log perubahan data
                                current_data = self.trend_data.get(symbol, {})
                                if (current_data.get('trend') != trend_data['trend'] or 
                                    current_data.get('signal') != trend_data['signal']):
                                    logger.info(f"Trend updated for {symbol}: {trend_data}")
                                
                                self.trend_data[symbol] = trend_data
                        else:
                            logger.warning(f"Failed to get trend data for {symbol}")
                    except Exception as e:
        ...
        with self.data_lock:
            for symbol in self.symbols:
                ...
                asset = {
                    ...
                    'trend_change': f"{trend_change:.2f}%" if trend_change else "0.00%",
