...
class SignalDetector:
    ...
    def _get_position_info(self, order_id: str) -> Optional[Dict]:
        """Dapatkan informasi posisi dengan validasi quantity"""
        with self.db_semaphore:
            conn = self._get_db_connection()
            if not conn:
                return None
            ...
            try:
                ...
                try:
                    ...
                    if quantity <= 0:
                        logger.error(f"Invalid quantity for order {order_id}: {row.qty}")
                        return None
                except (TypeError, ValueError):
                    logger.error(f"Invalid quantity format for order {order_id}")
                    return None

                return {
                    'symbol': row.symbol,
                    'position_side': row.posisi.upper(),
                    'quantity': quantity,
