...
class SignalDetector:
    ...
    def _fetch_btc_price_index(self):
        """Memanggil stored procedure sp_indicatorBTC dan menyimpan hasil price_index dan recomendacion_id."""
        conn = None
        try:
            with self.db_semaphore:
                conn = self._get_db_connection()
                if not conn:
                    logger.error("Koneksi database gagal di _fetch_btc_price_index")
                    return

                cursor = conn.cursor()
                cursor.execute("EXEC sp_indicatorBTC")
                row = cursor.fetchone()

                if row:
                    # Debugging: log semua atribut row
                    logger.info(f"Data dari sp_indicatorBTC: {dict(zip([column[0] for column in cursor.description], row))}")

                    
                    # Asumsikan kolom 'price_index' dan 'recomendacion_id' ada
                    price_index = getattr(row, 'price_index', None)
                    recomendasi_btc = getattr(row, 'recomendasi_id', None)
                    
                    if price_index is not None:
                        with self.data_lock:
                            self.btc_price_index = float(price_index)
                            self.btc_index_last_updated = datetime.utcnow()
                        logger.info(f"Price Index BTC diperbarui: {self.btc_price_index:.4f}")
                    else:
                        logger.error("Kolom 'price_index' tidak ditemukan di hasil sp_indicatorBTC")
                    
                    if recomendasi_btc is not None:
                        with self.data_lock:
                            self.btc_recomendasi_btc = recomendasi_btc
                        logger.info(f"BTC Recommendation ID diperbarui: {recomendasi_btc}")
                    else:
                        logger.error("Kolom 'recomendacion_id' tidak ditemukan di hasil sp_indicatorBTC")
                else:
                    logger.warning("sp_indicatorBTC tidak mengembalikan data")
        except Exception as e:
            logger.error(f"Gagal mengambil BTC price index: {e}", exc_info=True)
        finally:
            if conn:
                conn.close()
    ...
    def _process_closed_bar(self, symbol: str):
        ...
        if symbol == "BTCUSDT":
            self._fetch_btc_price_index()

        logger.info(f"Reset akumulator dan update OI untuk {symbol}. ATR14: {atr14:.4f}")
    
    def _periodic_btc_index_updater(self):
        """Thread untuk memperbarui BTC price index secara periodik"""
        while not self.shutdown_event.wait(300):  # Update setiap 5 menit
            self._fetch_btc_price_index()
    ...
    def _signal_detector(self):
        ...
        while not self.shutdown_event.is_set():
            try:
                time.sleep(self.SIGNAL_DETECTION_INTERVAL)
                now = datetime.utcnow()
                current_candle = now.replace(second=0, microsecond=0, minute=(now.minute // 15) * 15)
                ...
                for symbol in self.symbols:
                    ...
                    try:
                        # Ambil data real-time dengan lock
                        with self.data_lock:
                        ...
                        # ===== SISTEM SKORING REAL-TIME =====
                        score = 0

                        # 1. Perubahan Open Interest
                        if previous_oi_val > 0:
                            oi_change_percent = ((current_oi - previous_oi_val) / previous_oi_val) * 100
                            if oi_change_percent > 2: score += 1
                            elif oi_change_percent < -2: score -= 1

                        # 2. Dominasi Volume
                        if buy_ratio > 60: score += 1
                        elif sell_ratio > 60: score -= 1
                        ...
                        if len(fund_hist) >= 10:
                            ...
                            if deviation < -0.0003: score += 1
                            elif deviation > 0.0003: score -= 1
                        else:
                            if funding_rate < 0: score += 1
                            elif funding_rate > 0.0005: score -= 1

                        # 4. Likuidasi Squeeze
                        liq_sell_usd = liq.get('sell', 0) * mark_price
                        liq_buy_usd = liq.get('buy', 0) * mark_price
                        if liq_sell_usd > 50000: score += 1
                        elif liq_buy_usd > 50000: score -= 1
                        ...
                        if current_high > prev_high: score += 1
                        if current_low < prev_low: score -= 1

                        # ===== INTEGRASI DENGAN STORED PROCEDURE =====
                        recomendacion_id = indicator.get('recomendacion_id', 'HOLD')
                        sp_score = indicator.get('score_id', 0)
                        
                        # Faktor konfirmasi dari stored procedure
                        if recomendacion_id == 'LONG' and btc_index_val >= 0.20 and btc_index_val <= 0.70: score += 2
                        elif recomendacion_id == 'LONG' and btc_index_val > 0.70: score -= 2
                        elif recomendacion_id == 'SHORT' and btc_index_val <= -0.20 and btc_index_val <= -0.70: score -= 2
                        elif recomendacion_id == 'SHORT' and btc_index_val < -0.70: score += 2
                        elif recomendacion_id == 'HOLD': score = max(min(score, 5), -5)  # Batasi sinyal lemah

                        # ===== LOGIKA SINYAL AKHIR =====
                        signal = 'HOLD'
                        
                        # Aturan utama dengan konfirmasi BTC
                        if score >= 6:
                            if btc_recommendation == 'LONG' and btc_index_val > 0.20:
                                signal = 'LONG'
                            elif btc_index_val > -0.10:  # Toleransi penurunan BTC kecil
                                signal = 'LONG'
                        elif score <= -6:
                            if btc_recommendation == 'SHORT' and btc_index_val < -0.20:
                                signal = 'SHORT'
                            elif btc_index_val < 0.10:  # Toleransi kenaikan BTC kecil
                                signal = 'SHORT'
                        ...
                        if order_book.get('bids') and order_book.get('asks'):
                            ...
                            # Validasi spread dan depth
                            if spread_ratio > 0.001 or atr14 > 0 and spread > atr14 * 0.3:
                                valid_signal = False

                        # ===== SIMPAN SINYAL YANG VALID =====
                        if valid_signal and signal in ["LONG", "SHORT"]:
