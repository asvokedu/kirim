...
class SignalDetector:
    ...
    def _cancel_binance_order(self, symbol: str, order_id: str) -> Tuple[bool, str]:
        """Batalkan order di Binance Futures dan kembalikan (success, message)"""
        try:
            # Konversi order ID ke integer jika memungkinkan
            try:
            ...
            # Handle response lebih baik
            if response.status_code == 200:
                return True, "Order canceled successfully"
            
            # Cek jika order sudah tidak ada
            if response.status_code == 400:
                error_data = response.json()
                error_code = error_data.get('code')
                error_msg = error_data.get('msg', '')
                
                if error_code == -2011 or "Unknown order sent" in error_msg:
                    return False, f"Order does not exist: {error_msg}"
                if error_code == -2013 or "Order does not exist" in error_msg:
                    return False, f"Order does not exist: {error_msg}"
    ...
    def _update_order_status(self, order_id: str, status: int, close_price: float) -> bool:
        """Update order status dengan penanganan error lebih baik"""
        for attempt in range(3):  # Tambahkan retry
            try:
                with self.db_semaphore:
                    conn = self._get_db_connection()
                    if not conn:
                ...
                logger.error(f"General error updating order status: {e}")
                time.sleep(0.5 * (attempt + 1))
            finally:
                try:
                    conn.close()
    ...
    def _refresh_open_orders_cache(self):
        ...
        try:
            ...
            # PERBAIKAN: Kirim update ke klien hanya jika berhasil
            if self.open_orders_cache.get('status') == 'success':
                self.socketio.emit('open_orders_update', self.open_orders_cache, namespace='/')
        except Exception as e:
            logger.error(f"Error refreshing open orders cache: {e}")
    ...
    def cancel_order(self):
        ...
        try:
            ...
            # Handle order ID 'PENDING' khusus
            if order_id.lower() == 'pending':
                return jsonify({
                    'success': False,
                    'message': 'Cannot cancel PENDING orders'
                }), 400

            logger.info(f"Processing cancel order for {symbol} - ID: {order_id}")
            
            # 1. Batalkan order di Binance
            cancel_success, cancel_msg = self._cancel_binance_order(symbol, order_id)
            
            # PERBAIKAN: Tangani kasus di mana order sudah tidak ada
            if "does not exist" in cancel_msg or "Unknown order sent" in cancel_msg:
                # Update status di database ke canceled
                self._update_order_status(
                    order_id=order_id,
                    status=3,
                    close_price=0
                ...
                # Refresh data dan kirim respon sukses
                self._refresh_open_orders_cache()
                return jsonify({
                    'success': True,
                    'message': 'Order already canceled or does not exist'
                })
            ...
            update_success = self._update_order_status(
                order_id=order_id,
                status=3,  # Status: Canceled
                close_price=0
            )

            if not update_success:
                # PERBAIKAN: Log warning tapi tetap anggap sukses
                logger.warning("Database update failed after successful Binance cancel")
                return jsonify({
                    'success': True,  # Tetap dianggap sukses
                    'message': f"Order canceled on Binance but database update failed: {cancel_msg}"
                })

            # PERBAIKAN: Refresh data dan kirim update ke frontend
            self._refresh_open_orders_cache()
