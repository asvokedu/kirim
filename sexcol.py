...
class SignalDetector:
    ...
    def close_order(self):
        ...
        try:
            ...
            if not data or 'order_id' not in data or 'symbol' not in data:
                return jsonify({'success': False, 'msg': 'Invalid request format'}), 400
    ...
    def _get_position_info(self, binance_order_id: str) -> Optional[Dict]:
        """Dapatkan informasi posisi dengan validasi tambahan"""
        with self.db_semaphore:
            conn = self._get_db_connection()
            if not conn:
                return None
            ...
            try:
                ...
                try:
                    ...
                    if quantity <= 0:
                        logger.error(f"Invalid quantity for order {binance_order_id}: {row.qty}")
                        return None
                except (TypeError, ValueError):
                    logger.error(f"Invalid quantity format for order {binance_order_id}")
                    return None

                # Konversi posisi yang konsisten
                position_map = {
                    'BUY': 'LONG',
                    'SELL': 'SHORT',
                    'LONG': 'LONG',
                ...
                position = str(row.posisi).upper()
                position_side = position_map.get(position)
                
                if not position_side:
                    logger.error(f"Unknown position type: {position}")
                    return None
                ...
                logger.error(f"Database error in _get_position_info: {e}")
                return None
            finally:
                conn.close()

    def _close_binance_position(self, symbol: str, position_side: str, quantity: float) -> Dict:
        """Tutup posisi di Binance Futures dengan penanganan error lebih baik"""
        if not self.BINANCE_API_KEY or not self.BINANCE_API_SECRET:
            return {'success': False, 'msg': 'Binance credentials not configured'}
        ...
        # Validasi sisi posisi
        if position_side not in ['LONG', 'SHORT']:
            return {'success': False, 'msg': f'Invalid position side: {position_side}'}
        ...
        try:
            params = {
                ...
                'quantity': quantity,
                'timestamp': int(time.time() * 1000),
                'reduceOnly': 'true'  # Pastikan hanya untuk menutup posisi
            }
            ...
            # Tangani response error secara spesifik
            if response.status_code != 200:
                try:
                    ...
                    # Penanganan khusus untuk error umum
                    if error_code == -2010:  # Insufficient balance
                        return {'success': False, 'msg': 'Insufficient balance to close position'}
                    elif error_code == -2011:  # Unknown order
                        return {'success': False, 'msg': 'Order not found on Binance'}
                        
                    return {'success': False, 'msg': f"Binance Error {error_code}: {error_msg}"}
                except:
                    return {'success': False, 'msg': f"HTTP Error {response.status_code}"}

            order_data = response.json()
            
            # Dapatkan harga eksekusi
            if 'fills' in order_data and order_data['fills']:
            ...
            return {'success': False, 'msg': 'Binance API timeout'}
        except requests.exceptions.ConnectionError:
            return {'success': False, 'msg': 'Network connection error'}
        except Exception as e:
            logger.error(f"Unexpected error closing position: {str(e)}")
