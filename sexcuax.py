...
class SignalDetector:
    ...
    def submit_order(self):
        ...
        try:
            ...
            def sync_order_price():
                attempts = 0
                max_attempts = 18000  # Maksimal 18000 percobaan (15 jam, karena setiap percobaan tidur 3 detik)
                order_filled = False
                ...
                while attempts < max_attempts and not self.shutdown_event.is_set():
                    ...
                    if status in ['PARTIALLY_FILLED', 'FILLED']:
                        ...
                        if executed_qty > 0:
                            ...
                            update_success = self._submit_order_to_database(
                                ...
                                take_profit=take_profit,
                                binance_order_id=binance_order_id_actual,
                                initial=False  # TANDAI SEBAGAI UPDATE
                            )
                            if update_success:
                    ...
                    # Jika order dibatalkan atau kadaluarsa, keluar dari loop
                ...
                if not order_filled:
                    logger.warning(f"Order {binance_order_id_actual} belum terisi setelah {max_attempts} percobaan")
    ...
    def _get_position_info(self, binance_order_id: str) -> Optional[Dict]:
        """Dapatkan informasi posisi dengan konversi posisi yang benar"""
        with self.db_semaphore:
            conn = self._get_db_connection()
            if not conn:
                return None
            ...
            try:
                ...
                # Konversi posisi ke format standar
                position = str(row.posisi).upper().strip()
                position_map = {
                    "BUY": "LONG",
                    "SELL": "SHORT",
                    "LONG": "LONG",
                ...
                standardized_position = position_map.get(position)
                if not standardized_position:
                    logger.error(f"Invalid position value in DB: {position}")
                    return None
                ...
                try:
                    ...
                    if quantity <= 0:
                        logger.error(f"Invalid quantity: {row.qty}")
                        return None
                except (TypeError, ValueError):
                    logger.error(f"Invalid quantity format: {row.qty}")
                    return None

                return {
                    'symbol': row.symbol,
                    'position_side': standardized_position,
                    'quantity': quantity,
    ...
    # Tentukan sisi order dan positionSide
    if position_side == "LONG":
        side = "SELL"  # TUTUP POSISI LONG DENGAN SELL
        binance_position_side = "LONG"
    elif position_side == "SHORT":
        side = "BUY"   # TUTUP POSISI SHORT DENGAN BUY
        binance_position_side = "SHORT"
    else:
        return {'success': False, 'msg': 'Invalid position side'}

    try:
        # Gunakan dua strategi: pertama dengan positionSide, kedua tanpa jika gagal
        attempts = [
            {'params': {
                'symbol': symbol,
                'side': side,
                'type': 'MARKET',
                'quantity': quantity,
                'positionSide': binance_position_side,  # Coba dengan positionSide
                'timestamp': int(time.time() * 1000)
            }},
            {'params': {
                'symbol': symbol,
        ...
        for attempt in attempts:
            try:
                ...
                last_error = e
                # Jika error terkait positionSide, coba attempt berikutnya
        ...
        error_msg = f"All attempts failed: {str(last_error)}"
