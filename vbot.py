...
class FuturesTracker:
    ...
    def _vbot_update_trade_in_db(self, trade_id: int, price_close: float, binance_close_id: str, close_reason: str) -> bool:
        ...
        try:
            ...
            query = """
                ...
                WHERE id = ?
            """
            params = (price_close, datetime.utcnow(), binance_close_id, net_pnl, close_reason, trade_id)
            cursor.execute(query, params)
            ...
            conn.commit()
            logger.info(f"Posisi ditutup: ID={trade_id} @ {price_close:.5f}, close_id={binance_close_id}, Realized PnL: ${net_pnl:.2f}, Reason: {close_reason}")
            return True
        except Exception as e:
    ...
    def _vbot_create_session(self) -> requests.Session:
    ...
    def _vbot_fetch_valid_futures_symbols(self) -> Set[str]:
        try:
    ...
    def _vbot_load_symbol_list(self):
        ...
        self.valid_symbols = self._vbot_fetch_valid_futures_symbols()
    ...
    def _vbot_periodic_db_reloader(self):
        ...
        while not self.shutdown_event.is_set():
            ...
            new_data = self._vbot_fetch_symbols_from_db()
            if not new_data:
    ...
    def _vbot_fetch_api_data(self, url: str, params: Dict) -> Optional[Dict]:
    ...
    def _vbot_fetch_order_book_snapshot(self, symbol: str) -> Optional[Dict]:
        data = self._vbot_fetch_api_data(self.DEPTH_URL, {"symbol": symbol, "limit": self.ORDERBOOK_DEPTH_LEVEL})
        return {'bids': data.get('bids', []), 'asks': data.get('asks', []), 'timestamp': datetime.utcnow()} if data else None

    def _vbot_initialize_data_structures(self):
        with self.data_lock:
            for symbol in self.symbols:
    ...
    def _vbot_initialize_order_books(self):
        logger.info("Memulai inisialisasi order book awal...")
        threads = []
        def fetch_and_store(symbol):
            snapshot = self._vbot_fetch_order_book_snapshot(symbol)
            if snapshot:
                with self.data_lock:
    ...
    def _vbot_process_closed_bar(self, symbol: str):
        oi_data = self._vbot_fetch_api_data(self.OPEN_INTEREST_URL, {"symbol": symbol})
    ...
    def _vbot_handle_kline_stream(self, data):
        stream_data = data.get('data', {})
        if stream_data.get('e') == 'kline' and stream_data.get('k', {}).get('x'):
            symbol = stream_data['k']['s'].upper()
            threading.Thread(target=self._vbot_process_closed_bar, args=(symbol,), name=f"BarProc-{symbol}").start()

    def _vbot_handle_liquidation_stream(self, data):
        if isinstance(data, dict) and data.get('e') == 'forceOrder':
            self.liquidation_queue.put(data.get('o', {}))

    def _vbot_handle_trade_stream(self, data):
    ...
    def _vbot_handle_mark_price_stream(self, data):
    ...
    def _vbot_trade_processor(self):
        while not self.shutdown_event.is_set():
            try:
            ...
            try:
                ...
                if available_margin < required_margin:
                    logger.warning(
                        ...
                        f"Diperlukan: {required_margin:.2f} USDT"
                    )
                    with self.processing_lock:
                        self.processing_symbols.discard(symbol)
                    continue
    ...
    def _vbot_update_pending_order(self, order_id_db: int, execution_price: float, qty: float, binance_order_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                ...
                f"binance_order_id={binance_order_id}, posisi_ch={current_posisi_ch}, signal_ch={current_signal_ch}"
            )
            return True
        except Exception as e:
    ...
    def _vbot_display_dashboard(self, stdscr):
        curses.curs_set(0)
        stdscr.nodelay(True)
        curses.start_color()
                        ...
                        if symbol in self.open_positions:
                            ...
                            if trade_id in self.pnl_real_cache:
                                ...
                                if conn:
                                    try:
                                        cursor = conn.cursor()
                                        cursor.execute("SELECT pnl_real FROM view_td WHERE id = ?", (trade_id,))
                                        row = cursor.fetchone()
                                        if row:
    ...
    def run(self):
        ...
        self.session = self._vbot_create_session()
        ...
        stream_configs = [
            ...
            ('Kline', self.BASE_WS_URL + "/".join([f"{s.lower()}@kline_{self.INTERVAL}" for s in self.symbols]), self._vbot_handle_kline_stream),
            ('Trade', self.BASE_WS_URL + "/".join([f"{s.lower()}@aggTrade" for s in self.symbols]), self._vbot_handle_trade_stream),
            ('Depth', self.BASE_WS_URL + "/".join([f"{s.lower()}@depth{self.ORDERBOOK_DEPTH_LEVEL}@500ms" for s in self.symbols]), self._vbot_handle_depth_stream),
        ]

        threads = [
            threading.Thread(target=self._vbot_liquidation_processor, name="LiqProcessor", daemon=True),
            threading.Thread(target=self._vbot_trade_processor, name="TradeProcessor", daemon=True),
            threading.Thread(target=self._vbot_depth_processor, name="DepthProcessor", daemon=True),
            threading.Thread(target=self._vbot_periodic_data_updater, name="PeriodicUpdater", daemon=True),
            threading.Thread(target=self._vbot_trading_executor, name="TradingExecutor", daemon=True),
            threading.Thread(target=self._vbot_position_monitor, name="PositionMonitor", daemon=True),
            threading.Thread(target=self._vbot_leverage_retry_manager, name="LeverageRetry", daemon=True),
            threading.Thread(target=self._vbot_pending_order_executor, name="PendingOrderExecutor", daemon=True),
            threading.Thread(target=self._vbot_periodic_symbol_reloader, name="SymbolReloader", daemon=True),
            threading.Thread(target=self._vbot_periodic_db_reloader, name="DBReloader", daemon=True),
