...
class SignalDetector:
    ...
    def _update_aset_table(self, symbol: str, position: str, score: int):
        """Update tabel T_Aset dengan posisi dan signal terbaru"""
        ...
        try:
            ...
            if row and row[0] > 0:
                ...
                cursor.execute("""
                    UPDATE T_Aset 
                    SET posisi = ?, signal = ?, last_updated = ?
                    WHERE symbol = ?
                """, (position, score, datetime.utcnow(), symbol))
            else:
                # Insert new record
                cursor.execute("""
                    INSERT INTO T_Aset (symbol, posisi, signal, last_updated)
                    VALUES (?, ?, ?, ?)
                """, (symbol, position, score, datetime.utcnow()))
                
            conn.commit()
            logger.info(f"Updated T_Aset for {symbol}: {position} ({score})")
            return True
        except Exception as e:
                ...
                for symbol in self.symbols:
                    ...
                    try:
                        ...
                        if last_signal != signal or last_score != score:
                            self._update_aset_table(symbol, signal, score)
