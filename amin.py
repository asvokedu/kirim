...
class SignalDetector:
    ...
    def __init__(self):
        ...
        self.trend_data = {}  # {symbol: {trend, signal, change}}
    ...
    def _fetch_trend_data(self, symbol: str) -> Optional[Dict[str, Any]]:
        """
        Mengambil data tren dari stored procedure di database (versi perbaikan).
        """
        try:
            # 1. Membuat koneksi ke database
            with self._get_db_connection() as conn:
                with conn.cursor() as cursor:
                    # 2. Query untuk mengeksekusi stored procedure
                    # Query ini sama dengan yang Anda berikan
                    sql = """
                        DECLARE @signal NVARCHAR(10), @trend NVARCHAR(10), @change FLOAT;
                        EXEC dbo.sp_scalping_signal_15m @symbol = ?, @signal_out = @signal OUTPUT, @trend_out = @trend OUTPUT, @percent_change_out = @change OUTPUT;
                        SELECT @signal AS ScalpingSignal, @trend AS Trend, @change AS PercentChange;
                    """
                    
                    # 3. Eksekusi query dengan parameter simbol
                    cursor.execute(sql, (symbol,))
                    row = cursor.fetchone() # Ambil satu baris hasil
                    
                    # 4. Proses hasil jika ada
                    if row:
                        # Ambil data dari hasil query menggunakan nama kolom yang benar
                        # ('ScalpingSignal', 'Trend', 'PercentChange') lalu susun
                        # ke dalam format dictionary yang dibutuhkan aplikasi.
                        return {
                            'signal': row.ScalpingSignal,
                            'trend': row.Trend,
                            'change': row.PercentChange
                        }
                        
        except pyodbc.Error as ex:
            # Penanganan error spesifik untuk database
            sqlstate = ex.args[0]
            logger.error(f"Database error saat mengambil tren untuk {symbol}: {sqlstate} - {ex}")
        except Exception as e:
            # Penanganan error umum
            logger.error(f"Error tidak diketahui saat mengambil tren untuk {symbol}: {e}")
            
        # 5. Kembalikan None jika terjadi error atau tidak ada data
        # Ini mencegah aplikasi crash dan menandakan data tidak berhasil diambil.
        return None

    # Thread baru untuk update data trend
    def _periodic_trend_updater(self):
        """Thread untuk memperbarui data trend secara periodik"""
        while not self.shutdown_event.wait(15):  # Update setiap 15 detik
            try:
                logger.info("Memulai pembaruan data trend...")
                ...
                for symbol in self.symbols:
                    ...
                    try:
                        # Gunakan fungsi asli yang memanggil stored procedure
                        trend_data = self._fetch_trend_data(symbol)
                        if trend_data:
                            with self.trend_lock:
                                self.trend_data[symbol] = trend_data
                    except Exception as e:
                        logger.error(f"Error update trend untuk {symbol}: {e}")
                
                logger.info("Pembaruan data trend selesai.")
            except Exception as e:
                logger.error(f"Error pada trend updater: {e}")
            ...
            try:
                ...
                for symbol in self.symbols:
                    ...
                    try:
                        ...
                        with self.trend_lock:
                            trend_info = self.trend_data.get(symbol, {})
                            trend = trend_info.get('trend', 'NEUTRAL')
                            trend_signal = trend_info.get('signal', 'HOLD')
                            trend_change = trend_info.get('change', 0.0)
                        ...
                        # Faktor 6: Trend Confirmation
                        if trend == 'UPTREND':
                            score += 2  # Bonus untuk LONG
                            # Jika sinyal scalping juga bullish, tambah bonus
                            if trend_signal == 'BUY':
                                score += 1
                        elif trend == 'DOWNTREND':
                            score -= 2  # Bonus untuk SHORT
                            # Jika sinyal scalping juga bearish, tambah bonus
                            if trend_signal == 'SELL':
                                score -= 1
                        
                        # Faktor 7: Trend Strength
                        if abs(trend_change) > 0.5:  # Perubahan signifikan
                            if trend == 'UPTREND' and trend_change > 0:
                                score += 1
                            elif trend == 'DOWNTREND' and trend_change < 0:
                                score -= 1
                        ...
                        with self.data_lock:
                            if symbol in self.display_data:
                                self.display_data[symbol]['trend'] = trend
                                self.display_data[symbol]['trend_signal'] = trend_signal
                                self.display_data[symbol]['trend_change'] = trend_change    
    ...
    def get_formatted_data(self):
        ...
        # Ambil snapshot trend data dengan lock
        with self.trend_lock:
            trend_data_snapshot = self.trend_data.copy()
        ...
        with self.data_lock:
            for symbol in self.symbols:
                ...
                # PERBAIKAN: Ambil data trend dari snapshot
                trend_info = trend_data_snapshot.get(symbol, {})
                trend = trend_info.get('trend', 'NEUTRAL')
                trend_signal = trend_info.get('signal', 'HOLD')
                trend_change = trend_info.get('change', 0.0)
                ...
                asset = {
                    ...
                    # PERBAIKAN: Gunakan data trend yang sudah diambil
                    'trend': trend,
                    'trend_signal': trend_signal,
                    'trend_change': f"{trend_change:.2f}%",
    ...
    def run(self):
        ...
        threads = [
            ...
            threading.Thread(target=self._periodic_trend_updater, name="TrendUpdater", daemon=True),
