...
class SignalDetector:
    ...
    def _fetch_btc_price_index(self):
        """Memanggil stored procedure sp_indicatorBTC dan menyimpan hasil price_index."""
        conn = None
        try:
            with self.db_semaphore:
                conn = self._get_db_connection()
                ...
                if row:
                    ...
                    if hasattr(row, 'price_index'):
                        with self.data_lock:
                            self.btc_price_index = float(row.price_index)
                ...
                for symbol in self.symbols:
                    ...
                    try:
                        ...
                        if score not in [-6, 6] and abs(score) <= 6:
                            ...
                            elif score == 6 and buy_liq_usd== 0 and sell_liq_usd== 0 and btc_index_val > 0.005 :
                                signal = "LONG"
