...
class SignalDetector:
    ...
    def __init__(self):
        ...
        # ===== PENAMBAHAN UNTUK TREND DETECTION =====
        # Struktur data untuk menyimpan informasi trend
        self.trend_data: Dict[str, Dict] = {}  # {symbol: {trend, signal, change}}
        self.trend_lock = threading.Lock()

    # ===== FUNGSI BARU UNTUK TREND DETECTION =====
    ...
    # ===== MODIFIKASI FUNGSI YANG SUDAH ADA UNTUK INTEGRASI TREND =====
    @login_required
    ...
    def _initialize_data_structures(self):
        with self.data_lock:
            for symbol in self.symbols:
                ...
                # Inisialisasi data trend
                self.display_data[symbol]['trend'] = 'NEUTRAL'
                self.display_data[symbol]['trend_signal'] = 'HOLD'
                self.display_data[symbol]['trend_change'] = 0.0
            ...
            try:
                ...
                for symbol in self.symbols:
                    ...
                    try:
                        ...
                        # === FAKTOR BARU: TREND CONFIRMATION ===
                        if trend == 'UPTREND':
                            # Bonus untuk LONG
                            score += 2
                        ...
                        # Simpan informasi trend ke display data
                        with self.data_lock:
        ...
        with self.data_lock:
            for symbol in self.symbols:
                ...
                asset = {
                    ...
                    'trend_signal': d.get('trend_signal', 'HOLD'),
                    'trend_change': d.get('trend_change', 0.0),
                    'updated': True
                }
                assets.append(asset)
    ...
    def run(self):
        ...
        # Muat data trend awal
        logger.info("Memulai inisialisasi data trend...")
        for symbol in self.symbols:
            trend_data = self._fetch_trend_data(symbol)
            if trend_data:
                with self.trend_lock:
                    self.trend_data[symbol] = trend_data
        logger.info("Inisialisasi data trend selesai.")
        ...
        threads = [
            ...
            threading.Thread(target=self._periodic_trend_updater, name="TrendUpdater", daemon=True)  # Thread baru untuk trend
        ]
