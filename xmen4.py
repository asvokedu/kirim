...
class SignalDetector:
    ...
    def __init__(self):
        ...
        self.AUTOBOT_MIN_TARGET_PROFIT = 0.25  # USD
        self.AUTOBOT_TRAILING_DISTANCE = 0.05  # USD
    ...
    def _get_atr_values(self, symbol: str) -> Tuple[float, float]:
        """Mengambil nilai ATR14 dan ATR2 dari database"""
        current_time = time.time()
        ...
        with self.db_semaphore:
            ...
            try:
                cursor = conn.cursor()
                cursor.execute("EXEC sp_calculate_atr14 @symbol=?",
                              (symbol))
                row = cursor.fetchone()

                if row:
                    atr14 = float(row.atr_14) if hasattr(row, 'atr_14') else 0.0
                    atr2 = float(row.atr_2) if hasattr(row, 'atr_2') else 0.0
            except Exception as e:
                logger.error(f"Gagal mengambil ATR values untuk {symbol}: {e}")
            finally:
                conn.close()

        # Update cache
        with self.atr_cache_lock:
            self.atr_cache[symbol] = ((atr14, atr2), current_time)
    ...
    def _process_closed_bar(self, symbol: str):
        ...
        atr14, atr2 = self._get_atr_values(symbol)
        ...
        with self.data_lock:
            ...
            # Update nilai ATR14
            self.atr_values[symbol] = (atr14, atr2)
