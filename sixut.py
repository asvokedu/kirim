...
class SignalDetector:
    ...
    def _cancel_binance_order(self, symbol: str, order_id: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            try:
                ...
                return False, f"Binance API error: {error_data.get('msg', str(e))}"
            except:
    ...
    def _update_order_status(self, order_id: str, status: int, close_price: float) -> bool:
        try:
            with self.db_semaphore:
                ...
                cursor.execute("""
                    UPDATE tran_order
                    SET status = ?, price_close = ?, timestamp_close = ?
                    WHERE binance_order_id = ?
                """, (status, close_price, datetime.utcnow(), str(order_id)))
                conn.commit()

                # Tambahkan log baris yang terpengaruh
                logger.info(f"Update status affected {cursor.rowcount} rows for order_id {order_id}")
                return cursor.rowcount > 0  # Pastikan setidaknya 1 baris terupdate

        except Exception as e:
            logger.error(f"Gagal update status order {order_id}: {e}")
            return False
    ...
    def cancel_order(self):
        """Endpoint untuk membatalkan order di Binance"""
        try:
            data = request.get_json()
            logger.info(f"Cancel order request: {data}")

            if not data or 'order_id' not in data or 'symbol' not in data:
                return jsonify({'success': False, 'msg': 'Missing order_id or symbol'}), 400

            order_id = int(data['order_id'])
            symbol = data['symbol'].strip().upper()

            success, message = self._cancel_binance_order(symbol, order_id)

            if success:
                # Tandai status canceled di database
                self._update_order_status(
                    order_id=order_id,
                    status=3,  # 3 = Canceled
                    close_price=0
                ...
                logger.info(f"Order {order_id} on {symbol} successfully canceled")
                return jsonify({'success': True, 'msg': message})

            else:
                logger.warning(f"Gagal membatalkan order {order_id}: {message}")
