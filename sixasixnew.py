...
class SignalDetector:
    ...
    def _submit_order_to_database(self, symbol: str, side: str, order_type: str,
        ...
        # Tentukan status berdasarkan order type
        if initial:
            # Order limit: status 2 (Pending/Cancelable)
            # Order market: status 1 (Active)
            status = 2 if order_type.upper() == "LIMIT" else 1
        else:
            # Untuk update, pertahankan status sebelumnya
            status = None  # Akan diabaikan dalam query update
        ...
        with self.db_semaphore:
            ...
            try:
                ...
                if initial:
                    # Insert new order dengan status yang ditentukan
                    query = """
                    ...
                    params = (
                        symbol,
                        price,
                        status,  # Gunakan status yang ditentukan
                        side.upper(),
                        stop_loss,
                        take_profit,
                        datetime.utcnow(),
                    ...
                    # Update existing order (tidak mengubah status)
                    query = """
                        UPDATE tran_order
                ...
                if initial:
                    status_name = "Pending (Cancelable)" if status == 3 else "Active"
                    logger.info(f"Order disimpan: {symbol} {side} {quantity} @ {price} | Status: {status_name}")
                else:
            ...
            try:
                ...
                if status == 3:  # Canceled status
                    cursor.execute("""
                        ...
                        WHERE binance_order_id = ? AND status = 2
