...
class SignalDetector:
                    ...
                    # Handle khusus untuk order Limit yang belum terisi
                        ...
                        if (side == OrderSide.BUY and current_price > price * 1.005) or \
                          (side == OrderSide.SELL and current_price < price * 0.995):
                            ...
                            # Batalkan order limit - PERBAIKAN: Tangkap tuple (success, message)
                            cancel_success, cancel_msg = self._cancel_binance_order(symbol, int(binance_order_id_actual))
                            if cancel_success:
    ...
    def close_order(self):
        """Endpoint untuk menutup posisi di Binance dengan logging tambahan"""
        try:
            data = request.get_json()
            ...
            if not data or 'order_id' not in data or 'symbol' not in data:
                return jsonify({
                    'success': False, 
                    'msg': 'Missing required fields: order_id and symbol'
                }), 400
            ...
            if not position_info:
                return jsonify({
                    'success': False,
                    'msg': f'Active position not found for ID: {order_id}'
                }), 404

            logger.info(f"Position info: {position_info}")
    ...
    def _close_binance_position(self, symbol: str, position_side: str, quantity: float) -> Dict:
        ...
        if position_side not in valid_positions:
        return {
            'success': False, 
            'msg': f'Invalid position side: {position_side}. Valid options: {", ".join(valid_positions)}'
        }}

        try:
            # HAPUS PARAMETER 'reduceOnly' KARENA TIDAK DIPERLUKAN UNTUK MARKET ORDER
            params = {
                'symbol': symbol,
                'side': side,
