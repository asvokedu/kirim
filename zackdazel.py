...
class SignalDetector:
    ...
    def _save_signal(self, symbol: str, mark_price: float, position: str, score: int, candle_timestamp: datetime) -> bool:
        """
        Simpan sinyal ke database dengan status=2 (pending)
        """
        try:
            logger.info(f"Attempting to save signal: {symbol} {position} @ {mark_price} (score={score})")
            
            with self.db_semaphore:
                conn = self._get_db_connection()
                if not conn:
                    logger.error("Database connection failed in _save_signal")
                    return False

                cursor = conn.cursor()

                # 1. Cek apakah ada posisi aktif
                cursor.execute("""
                    SELECT COUNT(*)
                    FROM tran_TF
                ...
                if active_positions > 0:
                    logger.info(f"Skip save signal - Active position exists for {symbol}")
                    return False

                # 2. Cek sinyal pending di candle yang sama
                cursor.execute("""
                    SELECT COUNT(*)
                    FROM tran_TF
                    WHERE symbol = ?
                ...
                if pending_signals > 0:
                    logger.info(f"Skip save signal - Pending signal exists for {symbol} in current candle")
                    return False

                # 3. Cek periode candle berbeda
                last_candle = self.last_candle_timestamps.get(symbol)
                if last_candle and last_candle == candle_timestamp:
                    logger.info(f"Skip save signal - Same candle period for {symbol}")
                    return False
                ...
                conn.commit()
                logger.info(f"Signal saved: {symbol} {position} @ {mark_price:.5f} (score={score})")
            ...
            logger.error(f"Gagal menyimpan sinyal: {e}", exc_info=True)
            return False
        finally:
            if conn:
                conn.close()
            ...
            try:
                ...
                for symbol in self.symbols:
                    ...
                    try:
                        ...
                        if price_change_pct > 0.3:  # Kenaikan > 0.3%
                            score += 4
                        elif price_change_pct < -0.3:  # Penurunan > 0.3%
                            score -= 4
                        ...
                        if previous_oi_val > 0:
                            ...
                            if oi_change > 1.5:
                                score += 2 if price_change_pct > 0 else -2
                            elif oi_change < -1.5:
                                score -= 2 if price_change_pct > 0 else 2
                        ...
                        if btc_recommendation == 'LONG' and btc_index_val > 0.02:
                            score += 2
                        elif btc_recommendation == 'SHORT' and btc_index_val < -0.02:
                            score -= 2
                        ...
                        if signal in ["LONG", "SHORT"]:
                            ...
                            # Jika autobot aktif dan sinyal berhasil disimpan, buka posisi
                            if saved and self.autobot_enabled:
                                self._autobot_open_position(
                                    symbol, 
                                    signal, 
                                    mark_price, 
                                    current_candle
                        ...
                        # PERBAIKAN: SELALU UPDATE REALTIME SIGNAL
                        # (termasuk untuk sinyal HOLD)
                        with self.signal_lock:
                            self.last_signals[symbol] = signal
                            self.current_scores[symbol] = score
                            logger.debug(f"Realtime signal updated: {symbol} = {signal} ({score})")
