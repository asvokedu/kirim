...
class SignalDetector:
    ...
    def _autobot_open_position(self, symbol: str, position: str, mark_price: float, current_candle: datetime):
        """Open position by autobot when conditions are met"""
        ...
        if notional < min_notional:
            logger.warning(f"Autobot: Notional value {notional} < min notional {min_notional} for {symbol}. Skipping.")
            return False
        
        # PERBAIKAN 2: Penanganan step size yang lebih akurat
        if step_size > 0:
            # Hitung jumlah step yang valid
            raw_steps = quantity / step_size
            steps = int(raw_steps)  # Ambil bilangan bulat
            quantity = steps * step_size  # Bulatkan ke kelipatan step size
            
            # Perbaiki presisi floating point
            precision = self._get_step_precision(step_size)
            quantity = round(quantity, precision)
        else:
            # Jika step_size 0, gunakan presisi 8 digit
            quantity = round(quantity, 8)
            
        # PERBAIKAN 3: Pastikan quantity tidak kurang dari min_qty
        if quantity < min_qty:
            logger.warning(f"Autobot: Quantity {quantity} is less than minQty {min_qty} for {symbol}. Using minQty.")
            quantity = min_qty
        
        # PERBAIKAN 4: Validasi akhir quantity
        if quantity <= 0:
            logger.error(f"Autobot: Invalid quantity {quantity} for {symbol}. Skipping.")
            return False
        ...
        if not order_result['success']:
            ...
            logger.error(f"Autobot: Order failed for {symbol}: {order_result.get('msg')}")
            return False
        ...
        if db_success:
            logger.info(f"Autobot: Order for {symbol} saved to database")
