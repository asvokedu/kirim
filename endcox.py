...
class SignalDetector:
    ...
    def _autobot_open_position(self, symbol: str, position: str, mark_price: float, current_candle: datetime):
        """
        Fungsi utama untuk membuka posisi secara otomatis.
        PERBAIKAN UTAMA: Logika penyesuaian kuantitas (quantity) yang sepenuhnya dirombak
                         untuk memenuhi aturan `stepSize` dan `minNotional` dari Binance,
                         yang merupakan akar penyebab error '400 Bad Request'.
        """
        ...
        if self.autobot_last_open.get(symbol) == current_candle:
            logger.info(f"Autobot: Posisi untuk {symbol} di candle saat ini sudah dibuka. Melewati.")
            return False
        ...
        logger.info(f"Autobot [{symbol}]: Info -> StepSize: {step_size}, MinQty: {min_qty}, MinNotional: {min_notional}")
        ...
        # Place the order
        logger.info(f"Autobot: Menempatkan order {position} untuk {symbol}, Qty: {final_quantity}, Notional: ${final_quantity * mark_price:.2f}")
        ...
        # --- Lanjutan proses penyimpanan ke DB (tidak berubah) ---
        ...
        db_success = self._submit_order_to_database(
            symbol=symbol, side=posisi_db, order_type=OrderType.MARKET.value,
            quantity=final_quantity, price=mark_price, leverage=50,
    ...
    def _sync_order_price(self, binance_order_id: str, symbol: str):
        ...
        attempts = 0
        max_attempts = 15  # Tingkatkan jumlah percobaan jika perlu
        order_filled = False
        
        logger.info(f"Autobot: Memulai sinkronisasi harga untuk order ID Binance {binance_order_id} ({symbol})")
        ...
        while attempts < max_attempts and not self.shutdown_event.is_set():
            ...
            try:
                ...
                if not order_detail:
                    logger.warning(f"Autobot: Detail order {binance_order_id} tidak ditemukan pada percobaan ke-{attempts}. Mencoba lagi...")
                    continue
                    
                status = order_detail.get('status')
                logger.info(f"Autobot: Status order {binance_order_id} di Binance: {status} (Percobaan ke-{attempts})")
                
                # Handle filled orders
                if status in ['PARTIALLY_FILLED', 'FILLED']:
                    executed_qty = float(order_detail.get('executedQty', 0))
                    
                    # Pastikan executedQty lebih besar dari 0 sebelum mencoba mendapatkan avgPrice
                    if executed_qty > 0:
                        ...
                        if executed_price > 0:
                            ...
                            if update_success:
                                logger.info(f"Autobot: Berhasil memperbarui harga order {binance_order_id} ke {executed_price} di database.")
                                order_filled = True
                                break # Keluar dari loop setelah berhasil update
                            else:
                                logger.warning(f"Autobot: Gagal memperbarui harga untuk order {binance_order_id} di database (percobaan ke-{attempts}).")
                        else:
                            logger.warning(f"Autobot: avgPrice 0 untuk order {binance_order_id} meskipun FILLED/PARTIALLY_FILLED. Menunggu...")
                    else:
                        logger.info(f"Autobot: Order {binance_order_id} PARTIALLY_FILLED/FILLED tapi executedQty 0. Menunggu...")
                        
                elif status in ['CANCELED', 'EXPIRED']:
                    logger.warning(f"Autobot: Order {binance_order_id} dibatalkan atau kadaluarsa di Binance. Tidak perlu sinkronisasi harga.")
                    break # Keluar jika order dibatalkan/kadaluarsa
                elif status == 'NEW':
                    logger.info(f"Autobot: Order {binance_order_id} masih NEW di Binance. Menunggu pengisian...")
                elif status == 'PENDING_CANCEL':
                    logger.info(f"Autobot: Order {binance_order_id} sedang dalam proses pembatalan.")
                else:
                    logger.info(f"Autobot: Status order {binance_order_id} tidak dikenal: {status}. Menunggu...")
                    
            except Exception as e:
                logger.error(f"Autobot: Error saat sinkronisasi harga order {binance_order_id} (percobaan ke-{attempts}): {e}")
                # Pertimbangkan untuk menambahkan penanganan error spesifik di sini, misal:
                # Jika error adalah rate limit, tambahkan sleep lebih lama
                # Jika error adalah koneksi, coba lagi
        
        if not order_filled:
            logger.warning(f"Autobot: Order {binance_order_id} tidak terisi atau gagal disinkronkan setelah {max_attempts} percobaan.")
    ...
    def _fetch_symbol_info_map(self):
        ...
        PERBAIKAN: Sekarang juga mengambil 'stepSize' untuk validasi kuantitas.
        """
        ...
        try:
            logger.info("Memperbarui informasi simbol (minQty, minNotional, stepSize)...")
